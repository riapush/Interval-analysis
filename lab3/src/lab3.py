...
if __name__ == "__main__":
    ...
    for i in range (1, 10):
        ...
        Lambda = inv_midL(L)
        print('lambda', Lambda)
        q = eig_midL(abs(I - Lambda @ L))
        print(f'Спектральный радиус: {q}')
        x_av = X.mid
        F = F_x(x_av)
    ...
    for i in range(10):
        ...
        print(one, " & ", two)
        iveRect = plt.Rectangle((X_k[i][1].a, X_k[i][0].a),two , one, edgecolor='orange', facecolor='none', label='Брус ive', linewidth=1.5)
        plt.gca().add_patch(iveRect)
    x = np.arange(-2, 3.01, 0.01)
    y = np.sqrt(1 - pow(x, 2))
    plt.plot(x, y, color='magenta', linewidth=0.7)
    y = np.sqrt(x)
    plt.plot(x, y, color='indigo', linewidth=0.7)
    plt.grid()
    plt.xlim(0, 1)
    plt.ylim(0, 1)
    plt.show()
